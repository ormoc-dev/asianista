...
@section('title', 'Create Random Event')
@section('page-title', 'Create Random Event')

@section('content')
<div class="card">
    <div class="card-header">
        <h2 class="card-title">Create New Random Event</h2>
        ...
        <form action="{{ route('admin.random-events.store') }}" method="POST">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control @error('effect') is-invalid @enderror" 
                          id="effect" name="effect" rows="3" required>{{ old('effect') }}</textarea>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                <div class="form-group">
                    ...
                    <select class="form-control @error('event_type') is-invalid @enderror" 
                        ...
                        <option value="positive" {{ old('event_type') == 'positive' ? 'selected' : '' }}>Positive (Reward)</option>
                        <option value="negative" {{ old('event_type') == 'negative' ? 'selected' : '' }}>Negative (Penalty)</option>
                        <option value="neutral" {{ old('event_type') == 'neutral' ? 'selected' : '' }}>Neutral</option>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('target_type') is-invalid @enderror" 
                        ...
                        <option value="single" {{ old('target_type') == 'single' ? 'selected' : '' }}>Single Player</option>
                        <option value="all" {{ old('target_type') == 'all' ? 'selected' : '' }}>All Players</option>
                        <option value="pair" {{ old('target_type') == 'pair' ? 'selected' : '' }}>Pair</option>
                        <option value="random" {{ old('target_type') == 'random' ? 'selected' : '' }}>Random Selection</option>
                    </select>
                    @error('target_type')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px;">
                <div class="form-group">
                    ...
                    <input type="number" class="form-control @error('xp_reward') is-invalid @enderror" 
                           id="xp_reward" name="xp_reward" value="{{ old('xp_reward', 0) }}" min="0">
                    @error('xp_reward')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" class="form-control @error('xp_penalty') is-invalid @enderror" 
                           id="xp_penalty" name="xp_penalty" value="{{ old('xp_penalty', 0) }}" min="0">
                    @error('xp_penalty')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div style="display: flex; gap: 10px; margin-top: 30px;">
                ...
                <a href="{{ route('admin.random-events.index') }}" class="btn btn-secondary">Cancel</a>
            </div>
        </form>
    </div>
</div>
